 ...
 * HISTORY
 * $Log:	sed0.c,v $
 * Revision 1.5  89/04/03  16:42:55  dimitris
 * 	Added global variable check_length to indicate when
 * 	text() should make sure that the input length is less
 * 	than FNAMSIZE.
 * 	[89/04/03  16:42:16  dimitris]
 * 
 * Revision 1.3  89/01/27  10:40:34  dimitris
 * 	Added a test in text() to make sure that the internal
 * 	filename buffer does not overflow.
 * 
 *
 * 30-Jun-88  Joshua Raiff (raiff) at Carnegie-Mellon University
 *	Added which union field pointers are supposed to reference to allow
 *	compilation under HC
 *
 ****************************************************************************/
...
#ifdef CMUCS
int check_length = 0; /* used to indicate if text() reads a filename (1)
			or plain text that is allowed to be any length (0) */
...

{
	...
	for(;;) {
		...
		p++;
#ifdef CMUCS
		if ((check_length) && (counter++ >= FNAMSIZE)) {
