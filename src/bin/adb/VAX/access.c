 ...
 *	Changed MACH_VM conditional to MACH || MACH_VM
 * 14-Mar-87  Mike Accetta (mja) at Carnegie-Mellon University
 *	Conditioned out vtophys() for MACH_VM since it doesn't commpile
 *	and probably wouldn't work it it did.
 *
 **********************************************************************
 */
#endif	CMU
...
{
#if	MACH_VM || MACH
	errflg = "not (yet) possible with MACH_VM";
	return (-1);
#else	MACH_VM || MACH
	int oldaddr = addr;
	int v;
