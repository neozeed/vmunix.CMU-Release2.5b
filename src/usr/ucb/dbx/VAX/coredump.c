 ...
 *	Changed conditionals on MACH_VM to ( MACH_VM || MACH )
 *	Changed conditionals on MACH_TT to ( MACH_TT || MACH )
 *
 * 24-Sep-87  Mary Thompson (mrt) at Carnegie Mellon 
 *	Added define of KERNEL_FEATURES and redefined MAXSTKADDR for
 *	MACH_TT kernels.
 * 14-Mar-87  Mike Accetta (mja) at Carnegie-Mellon University
 *	Updated to compile under MACH_VM (without pcb_p[01]br) although
 *	I doubt it works.
 *
 **********************************************************************
 */
...
#if ( MACH_TT || MACH )
#define MAXSTKADDR (USRSTACK + sizeof(int[5]))	/* highest stack address */
#else
...
#if	( MACH_VM || MACH )
	    /*  I have no idea what this will do except compile ...  */
	    r = (Address) (v);
#else	( MACH_VM || MACH )
