 ...
 **********************************************************************
 * HISTORY
 *  7-Sep-89  Mary Thompson (mrt) at Carnegie-Mellon University
 *	Removed all the logging and posting stuff to produce a
 *	simplified version to put on the Mach 2.5 Release tape.
 *
 * $Log:	release.c,v $
...
do_release()
{
    ...
    if (status == 0) {
	printf ("Calling create_new_log_db(%s)\n",tologtmp);
