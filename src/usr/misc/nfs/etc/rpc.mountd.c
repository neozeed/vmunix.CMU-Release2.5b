/* @(#)rpc.mountd.c	1.2 86/11/20 NFSSRC */
#ifndef lint
 ...
 * HISTORY
 * $Log:	rpc.mountd.c,v $
 * Revision 2.0  89/06/15  15:31:58  dimitris
 *   Organized into a misc collection and SSPized
 * 
 * 
 * 26-May-89  Paul Parker (parker) at Carnegie-Mellon University
 *	Changed getfs syscall to nfs_getfh and added temporary code to
 *	warn about non-standard file system mounts.
 *
 **********************************************************************
 */
			...
			*l = '\0';
			if (bol) {
				...
				if (statb.st_ino != ROOTINO ||
					...
					fprintf(stdout,
					    "mountd: %s mounting special file system root\n",
					    str);
				}
