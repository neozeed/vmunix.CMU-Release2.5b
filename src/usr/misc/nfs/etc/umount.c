 ...
 * HISTORY
 * $Log:	umount.c,v $
 * Revision 2.1  89/07/19  09:11:36  dimitris
 * 	Changed to accept Ultrix type format for /etc/fstab.
 * 	[89/07/19  09:04:15  dimitris]
 * 
 * Revision 2.0  89/06/15  15:32:11  dimitris
 *   Organized into a misc collection and SSPized
 * 
 * 
 *
 *  5-Jan-89  Paul Parker (parker) at Carnegie-Mellon University
 *	Merged CMUCS changes listed in edit history below into NFS
 *	version.
 *
 * 06-Nov-80  Mike Accetta (mja) at Carnegie-Mellon University
 *	Changed to allow operator to unmount file systems.
 *
 **********************************************************************
 */
	...
	char *argv[];
{
...
#if CMUCS
				if (!(colon = index(mnt->mnt_fsname, ':'))) {
			          if (colon=index(mnt->mnt_fsname,"@"))
				     strcpy(mnt->mnt_fsname,
					     convert(mnt->mnt_fsname,"@",":"));
				}
#else CMUCS
	...
	for (; mntcur != NULL; mntcur = mntcur->mntl_next) {
		if (mntcur->mntl_mnt) {
        	...
        	if (strcmp( mntcur->mntl_mnt->mnt_type,MNTTYPE_NFS) == 0)
	          if (index( mntcur->mntl_mnt->mnt_fsname,':'))
        	    strcpy( mntcur->mntl_mnt->mnt_fsname,
			     convert( mntcur->mntl_mnt->mnt_fsname,":","@"));
#endif CMUCS
