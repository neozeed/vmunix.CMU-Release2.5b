 ...
 * HISTORY
 * $Log:	sh.glob.c,v $
 * Revision 1.2  89/01/19  16:32:03  dimitris
 * 	
 * 	Added the bug-fix suggested by Michael Greim to ensure 
 * 	that csh does not dump core when it has to do a double 
 * 	command substitution. This is accomplished by making 
 * 	sure that the child process forked by csh to evaluate
 * 	the command inside '`' starts with pargv = 0.
 * 	[89/01/19  16:28:39  dimitris]
 * 
 *
 */
