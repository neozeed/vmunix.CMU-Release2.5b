 ...
 * HISTORY
 * $Log:	tenex.c,v $
 * Revision 2.3  89/06/02  18:13:05  moore
 * 	Removed machine byte order-dependencies.  Kept things pretty much
 * 	in Vax order.
 * 	[89/06/02  18:12:06  moore]
 * 
 * Revision 2.2  89/05/09  11:36:31  moore
 * 	Added loadmacrofile and savemacrofile for user interface.
 * 	[89/05/09  11:34:18  moore]
 * 
 * Revision 2.1  88/10/03  14:05:12  berman
 * 	Added shell variable expansion to the search() function so that typing
 * 	ESC-l with a path like $foo/src/ will search the named directory
 * 	(if it exists) when the value of foo is set.  Will also allow
 * 	ESC-ESC file name completion, though, there must be a / delimiting
 * 	the shell variable in the command line.  Added function WipeLine()
 * 	to allow erasing a line without saving it in the kill buffer.
 * 	[88/09/16            berman]
 * 	
 * 	Added bindings for ANDREW users conditionalized under #if ANDREW.  
 * 	Added function DeleteCurrentCharOrEof() which can replace 
 * 	DeleteCurrentChar for binding to ^D under the ANDREW conditional. 
 * 	Added function  GnuTransposeChars from Stewart Clamen (clamen) 
 * 	to allow GNU style character transposition.
 * 	[88/08/23            berman]
 * 	
 * 	Removed "hidden" (static) modifiers from definitions for 
 * 	char * FunctionNames [] and CodeFunction FunctionPointers [].
 * 	This fixes these definitions for HC which seems to balk on statics
 * 	that are referenced as extern's.
 * 	[88/06/28            berman]
 * 
 *
 * 13-May-88  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Fixed "~" to be "$home", not the pw_dir of the current user.
 *
 * 08-May-88  Richard Draves (rpd) at Carnegie-Mellon University
 *	Changed include of sh.h: not everybody has . in their CPATH.
 *
 * 04-Aug-87  Doug Philips (dwp) at Carnegie-Mellon University
 *	Changed EnterCbreakMode to save the various terminal characters so that
 *	stty's will work.  Note, this does NOT affect the bindings.  Also added
 *	code to redisplay the prompt after printing a 'Editing' message.  Also
 *	fixed the "50" clear screen bug.
 *
 * 10-Nov-86  David VanRyzin (vanryzin) at Carnegie-Mellon University
 *	Fixed bug that divided by zero in print_by_column when a
 *	filename was larger than the screen width.
 *
...
char *fname;
{
  ...
  if (fd != -1) {
	...
	len = (KeyboardMacro[ curmacro].Tail-iter);
        ...
        temp_char = len;
	write( fd, (char *)&temp_char, 1);
	temp_char = len / 256;
	write( fd, (char *)&temp_char, 1);
	temp_char = len / (256 * 256);
	write( fd, (char *)&temp_char, 1);
	temp_char = len / (256 * 256 * 256);
	write (fd, (char *)&temp_char, 1);
...
/*	curpos += 4;		*/	/* And skip over it */
	macrosize =  ((unsigned int)*curpos++);
	macrosize += ((unsigned int)*curpos++) * 256;
	macrosize += ((unsigned int)*curpos++) * 256 * 256;
	macrosize += ((unsigned int)*curpos++) * 256 * 256 * 256;
...
#if ANDREW
    REGBIND (DeleteCurrentCharOrEof, CTRL_D);
#else /* ANDREW */
...
register char ** arglist;
{
    ...
    KeyBinding * keymap;
    extern CodeFunction FunctionPointers[];
...
#   define MAXITEMS 1024
    ...
    register numitems,
	    ...
	    dol_d,			/* True if $ is first part of path */
    ...
    char    tilded_dir[MAXPATHLEN + 1],	/* dir after ~ expansion */
	    ...
	    *org_word,			/* Original argument to be searched */
	...
	if ((p = rindex (word, '/')) == NULL || p < rindex (word, '$'))
		return (0);	/* leave if searching only $foo */

                  /* Expand $ in word into full path before checking for ~'s */
	dol_d = 1;      /* $ variable is part of path, will not do recognize */
	word = Dfix1(word);
    }
	...
	catn (org_word, extended_name+name_length, max_word_length);
					/* add extended name extension only */
	return (numitems);
    }
