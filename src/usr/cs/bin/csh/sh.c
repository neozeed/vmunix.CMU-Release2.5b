 ...
 * HISTORY
 * $Log:	sh.c,v $
 * Revision 1.4  89/08/15  15:12:53  mja
 * 	Fixed setjmp()/longjmp() compiler optimization problem on
 * 	PMAX.
 * 	[89/08/01            af]
 * 
 * 03-Aug-87  Doug Philips (dwp) at Carnegie-Mellon University
 *	Made CMUCS change to allow arguments to the login shell.  Also
 *	changed the location of the default profiles.
 *
...
#if	CMUCS
#define CSPROFILES "/usr/cs/lib/profiles/csh"
...
#ifdef	mips
/*
 * The mips compiler does not know about setjmp/longjmp.
 * To prevent it from moving around crucial variable assignments
 * we must simulate use of those variables at appropriate places.
 * Typically, a pretend use of a variable is places before
 * invocations of setjmp (aka setexit).
 */
static pretend_use(var)
{}
#endif	mips
