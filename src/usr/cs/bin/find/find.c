 ...
 * HISTORY
 * $Log:	find.c,v $
 * Revision 1.8  88/11/07  19:19:24  gm0w
 * 	Changes to Tahoe release:
 * 	
 * 	Added "-relative", "-depth #", "-maxdepth #", "-ctime #", and
 * 	"-match <name>" predicates.  Added "-nosymlink" and "-nomount"
 * 	(now obsolete because of "-follow" and "-xdev" predicates).
 * 	Disabled AMES code.  Fixed bug in stat of base directory for
 * 	Devstat.  Fixed "-follow" code to return from symlinks to
 * 	directories correctly.  Added error messages to stat failures.
 * 	Added "{}" patterns ala csh.
 * 	[88/11/07            gm0w]
 * 
 **********************************************************************
 */
...
#if	CMU
	while (argc > 1) {
		if (strcmp(Argv[1], "-nosymlink") == 0) {
			argc--; Argc--; Argv++;
		} else if (strcmp(Argv[1], "-nomount") == 0) {
			argc--; Argc--; Argv++;
			Xdev = 0;
		} else
			break;
...
#if	CMU
		if (Relative)
			Pathbase = Pathname + strlen(Pathname);
#endif	CMU
...
struct anode *e3() { /* parse parens and predicates */
	...
	if(EQ(a, "-name"))
		...
		return(mk(match, (struct anode *)b, (struct anode *)0));
	else if(EQ(a, "-depth"))
...
register struct { int f; char *pat; } *p; 
{
	return(amatch(PathName(), p->pat));
}
depth(p)
register struct { int f, depth, s; } *p; 
{
	return(scomp(Depth, p->depth, p->s));
}
Ctime(p)
	...
	char *name, *fname;
{
	...
	char *endofname;
#if	CMU
	char *updirp, updir[MAXPATHLEN+1];
#endif	/* CMU */
	...
	if (follow && (Statb.st_mode&S_IFMT)==S_IFLNK) {
...
ret:
	...
	if(chdir("..") == -1) {
		...
		*endofname = '\0';
#if	CMU
		fprintf(stderr, "find: bad directory: %s <%s>\n",
			errmsg(-1), name);
#else	/* CMU */
