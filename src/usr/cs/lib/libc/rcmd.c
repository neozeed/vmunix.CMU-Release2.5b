 ...
 *	At CMU CS, removed local domain handling code from _checkhost().
 *
 * 22-Nov-85  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Modified rresvport() to only use the last 128 privileged ports.
 *	Made ruserok() code more restrictive at CMU CS.  The superuser is
 *	never a valid remote user, /etc/hosts.equiv must contain the remote
 *	host name and ~/.rhosts must be owned by root, have a link count of
 *	one, a mode of 0600 and must be a regular file (not a symbolic link).
 *
 **********************************************************************
 */
#include <stdio.h>
	...
	char *ruser, *luser;
{
	...
	*p = '\0';
	...
	if (hostf == NULL)
		return(-1);
	if (_validuser(hostf, fhost, "", "", baselen) < 0) {
		(void) fclose(hostf);
		return(-1);
	}
	(void) fclose(hostf);
...
{
#if	CMUCS
	/* CMU has too many subdomains for this to be tractable... */
#else	CMUCS
